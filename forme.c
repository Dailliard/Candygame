...
#include "visual.h"
#include "forme.h"
//Fonction pour rechercher et gérer les formes dans la grille
void add(int tab[Y][X]){
    for(int y = 0; y < Y; y++){ // Parcourir les lignes
        for(int x = 0; x < X; x++){ // Parcourir les colonnes
            if(tab[y][x] == 0){ // Si la case est vide, ajouter un nouvel élément aléatoire
                tab[y][x] = rand() % 5 + 1;
            }
        }
    }
...
int malus(int tab[Y][X], Donnees *data){
    for(int i = 1; i < Y; i++){ // Parcourir les lignes
        for(int j = 1; j < X; j++){ // Parcourir les colonnes
            if(tab[i][j] != 0 && // Vérifier la présence d'un carré 2x2
                tab[i][j] == tab[i][j-1] &&
                tab[i][j] == tab[i-1][j] &&
                tab[i][j] == tab[i-1][j-1]){
                if(tab[i][j]==6){ // Si c'est une bombe, supprimer tous les éléments
                    for(int y = 0; y < Y; y++){ // Parcourir toutes les lignes
                        for(int x = 0; x < X; x++){ // Parcourir toutes les colonnes
                            data->Contrat[tab[y][x]][0]++;
                            tab[y][x] = 0;
                        }
                    }
                }
                else{ // Mettre à jour le contrat
                    if(data->Contrat[tab[i][j]][0]<=8)data->Contrat[tab[i][j]][0] = 0; // Sinon, décrémenter de 8
                    else data->Contrat[tab[i][j]][0] -= 8;
                    tab[i][j] = 0; // Supprimer les éléments du carré
                    tab[i][j-1] = 0; //
                    tab[i-1][j] = 0;
                    tab[i-1][j-1] = 0;
                }
...
int carre(int tab[Y][X], Donnees *data){
    for(int i = 3; i < Y; i++){ // Parcourir les lignes
        for(int j = 3; j < X; j++){ // Parcourir les colonnes
            if(tab[i][j]==tab[i][j-1] &&
                 tab[i][j]==tab[i][j-2] &&
                  tab[i][j]==tab[i][j-3]){

                if(tab[i][j]==tab[i-1][j] && // Vérifier les 4 éléments verticaux
                     tab[i][j]==tab[i-2][j] &&
                      tab[i][j]==tab[i-3][j]){
...
int croix(int tab[Y][X], Donnees *data){
    for(int i = 2; i < Y-2; i++){ // Parcourir les lignes
        for(int j = 2; j < X-2; j++){ // Parcourir les colonnes
            if(tab[i][j]==tab[i][j-1] && // Vérifier les 5 éléments horizontaux
                ...
                if(tab[i][j]==tab[i-1][j] && // Vérifier les 5 éléments verticaux
                    ...
                    int temp = tab[i][j];
                    for(int z = 0; z < X; z++){ // Parcourir les colonnes
                        if(tab[i][z] == temp){
                            tab[i][z] = 0;
                            data->Contrat[temp][0]++;
                        }
...
int horizontal7(int tab[Y][X], Donnees *data){
    for(int i = 0; i < Y; i++){ // Parcourir les lignes
        for(int j = 6; j < X; j++){ // Parcourir les colonnes
...
int horizontal6(int tab[Y][X], Donnees *data){
    for(int i = 0; i < Y; i++){ // Parcourir les lignes
        for(int j = 5; j < X; j++){ // Parcourir les colonnes
...
int horizontal4(int tab[Y][X], Donnees *data){
    for(int i = Y-1; i >=0; i--){ // Parcourir les lignes 
        for(int j = 3; j < X; j++){ // Parcourir les colonnes
...
int recherche_formes(int tab[Y][X], Donnees *data){
    ...
    do{ // Répéter jusqu'à ce qu'aucune forme ne soit trouvée
        ...
        count += croix(tab, data); // Tous les niveaux
        count += horizontal6(tab, data); // Tous les niveaux
        count += vertical6(tab, data); // Tous les niveaux
        count += horizontal4(tab, data); // Tous les niveaux
        count += vertical4(tab, data); // Tous les niveaux
    }while(count > 0);
    return 0;
}
